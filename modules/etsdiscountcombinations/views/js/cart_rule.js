...
$(document).ready(function(){
    ...
    $(document).on('click','input[name="rule_combination"]',function(){
        ets_dc_cart_rule.displayForm();
    });
...
var ets_dc_cart_rule = {
    displayForm: function(){
        if($('input[name="rule_combination"]').length)
        {
            $('.rule_combination.form-group').hide();
            $('.rule_combination.form-group.'+$('input[name="rule_combination"]:checked').val()).show();
            if($('input[name="rule_combination"]:checked').val()=='manual')
            {
                $('#cart_rule_restriction').prop('checked',true);
                toggleCartRuleFilter($('#cart_rule_restriction'));
            }
            else
            {
                $('#cart_rule_restriction').prop('checked',false);
                toggleCartRuleFilter($('#cart_rule_restriction'));
            }
        }
    },
    search_rule : function(){
        if ($('.dc_search_rule').length > 0 && typeof dc_link_search_rule !== "undefined")
        {
            ...
            dc_rule_autocomplete.autocomplete(dc_link_search_rule, {
                ...
                scrollHeight: 180,
                extraParams: {
                    excludeIds: $('input[name="id_cart_rule"]').val(),
                },
    ...
    addRule: function(data){
        var input_name = 'specific_rule_combination';
