...
var $document, $window, payplugModule = {
    ...
    form: {
        ...
        submit: function (event) {
            ...
            if (error) {
                return payplugModule.tools.popup.error(error);
            }
            ...
            form.props.query = $.ajax({
                ...
                success: function (result) {
                    if (typeof result.content != 'undefined') {
                        ...
                        var {oney, deferred,settings} = payplugModule;
                        settings.load();
                        $window.trigger('load');
        ...
        check: function () {
            ...
            var {installment, oney, deferred, form} = payplugModule;
    ...
    config: {
        ...
        refresh: function () {
            ...
            config.props.query = $.ajax({
                ...
                beforeSend: function () {
                    payplugModule.tools.loader.show($('.' + identifier));
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    if (textStatus != 'abort') {
                        alert('An error occurred while trying to refresh indicators. ' +
                            ...
                            'You will find more explanation in JS console.');
                        console.log(jqXHR, textStatus, errorThrown);
                        payplugModule.tools.loader.hide($('.' + identifier));
                    }
                },
                success: function (result) {
    ...
    login: {
        ...
        login: function (event) {
            ...
            var {login, settings} = payplugModule,
                {identifier} = login.props,
                data = {
                    _ajax: 1,
                    log: 1,
            ...
            login.props.query = $.ajax({
                ...
                success: function (result) {
                    if (typeof result.content != 'undefined' && result.content) {
                        ...
                        payplugModule.tools.popup.error(result.error);
                        payplugModule.tools.loader.hide($('.' + identifier));
                    }
                }
            });
    ...
    settings: {
        ...
        init: function () {
            var {settings, login} = payplugModule,
            ...
            settings.props.query = $.ajax({
                ...
                success: function (result) {
                    for (permission in result) {
                        ...
                        if ($input.length) {
                            var {switcher} = payplugModule.tools,
        ...
        live: function () {
            ...
            settings.props.query = $.ajax({
                ...
                success: function (response) {
                    if (response.result) {
                        settings.reset();
                        var {tools, bancontact} = payplugModule,
    ...
    standard: {
        ...
        init: function () {
            var {standard, deferred} = payplugModule,
    ...
    installment: {
        ...
        init: function () {
            var {installment, deferred} = payplugModule,
    ...
    deferred: {
        ...
        check: function () {
            var {standard, installment, deferred}= payplugModule;
        ...
        unavailable: function () {
            var {deferred, tools} = payplugModule,
                {switcher} = tools,
                data = {
                    _ajax: 1,
                    popin: 1,
