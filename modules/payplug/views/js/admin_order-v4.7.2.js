...
var $document, $window, payplugModule = {
    ...
    abort: {
        init: function () {
            var {abort} = payplugModule;
            $document.on('click', 'input[name=payplugSubmitAbort]', abort.call)
                .on('click', 'button[name=payplugConfirmAbort]', abort.confirm);
        },
        call: function (event) {
            event.preventDefault();
            var {popup} = payplugModule;
    ...
    capture: {
        init: function () {
            var {capture} = payplugModule;
            $document.on('click', 'input[name=payplugSubmitCapture]', capture.call);
        },
        call: function (event) {
            event.preventDefault();
            ...
            $.ajax({
                ...
                beforeSend: function () {
                    ...
                    $('input[name=payplugSubmitCapture]').unbind('click');
                },
                complete: function () {
                    $('.pp-capture .loader').hide();
                    $('input[name=payplugSubmitCapture]').bind('click', function (e) {
                        e.preventDefault();
                        callCapture();
                    });
                },
    ...
    refund: {
        init: function () {
            var {refund} = payplugModule;
            $document.on('click', 'input[name=payplugSubmitRefund]', refund.call);
        },
        call: function (event) {
            event.preventDefault();
            event.stopPropagation();
            ...
            $.ajax({
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    alert('An error occurred while trying to refund. ' +
                        ...
                        'You will find more explanation in JS console.');
                    console.log(jqXHR, textStatus, errorThrown);
                    $('input[name=submitpayplugRefund]').prop("disabled", false);
                },
                success: function (result) {
                    if (result.status == 'error') {
    ...
    update: {
        init: function () {
            var {update} = payplugModule;
            $document.on('click', 'input[name=payplugSubmitUpdate]', update.call);
        },
        call: function (event) {
            event.preventDefault();
            event.stopPropagation();
