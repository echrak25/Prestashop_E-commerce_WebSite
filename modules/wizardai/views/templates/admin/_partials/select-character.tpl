...
<div x-data="{
    ...
    <button  x-ref="selectButton" @click.prevent="selectOpen=!selectOpen"
             ...
             class="relative min-h-[38px] flex items-center w-full py-2 pl-3 pr-10 text-left bg-white border rounded-md shadow-sm cursor-default border-neutral-200/70 focus:outline-none  text-sm">
        <img :src="selectedItem.image" width="18" height="18" class="rounded-full mr-3" :alt="selectedItem.title">
        <span x-text="selectedItem.title ? selectedItem.title : '{$defaultText}'" class="truncate">{$defaultText|escape:'htmlall':'UTF-8'}</span>
    ...
    <ul x-show="selectOpen"
        ...
        <template x-for="item in selectableItems" :key="item.value">
            <li
                    @click="selectedItem=item; selectOpen=false; $refs.selectButton.focus(); $refs.modelInput._x_model.set(selectedItem.value)"
                ...
                <img :src="item.image" width="18" height="18" class="rounded-full mr-3" :alt="item.title">
