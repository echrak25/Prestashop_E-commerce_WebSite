...
if (!defined('_PS_VERSION_'))
    exit;
require_once(dirname(__FILE__) . '/classes/ets_plp_defines.php');
class Ets_productlistpagination extends Module
{
    public $hooks = array(
    ...
    {
        $this->name = 'ets_productlistpagination';
        ...
        $this->displayName = $this->l('Product list pagination');
        $this->description = $this->l('Add pagination to the product listing page on the PrestaShop store in 3 ways: pagination, "load more" button, or scroll to load');
$this->refs = 'https://prestahero.com/';
    ...
    public function uninstallOverrides()
    {
        ...
        {
            if(file_exists(_PS_OVERRIDE_DIR_.'classes/controller/ProductListingFrontController.php') && ($file_content = Tools::file_get_contents(_PS_OVERRIDE_DIR_.'classes/controller/ProductListingFrontController.php')) && Tools::strpos($file_content,'function')===false)
            {
                @unlink(_PS_OVERRIDE_DIR_.'classes/controller/ProductListingFrontController.php');
            }
            return true;
        }
    }
    ...
    {
        if(Configuration::get('ETS_PLP_ENABLED'))
        {
            $controller = Tools::getValue('controller');
            $pages = array('category','bestsales','manufacturer','newproducts','pricesdrop','search','supplier');
    ...
    public function _assignVariables()
    {
        return array(
            'ETS_PLP_TYPE_PAGINATION' => Configuration::get('ETS_PLP_TYPE_PAGINATION'),
            'ETS_PLP_BUTTON_LABEL' => Configuration::get('ETS_PLP_BUTTON_LABEL',$this->context->language->id) ? : $this->l('Load more'),
        );
    }
    public function hookDisplayOverrideTemplate($params)
    {
        if(Configuration::get('ETS_PLP_ENABLED'))
        {
            if(isset($params['template_file']) && Tools::strpos($params['template_file'],'catalog/listing/')===0)
            {
                $this->context->smarty->assign(
                    $this->_assignVariables()
                );
                return $this->getTemplatePath($params['template_file'].'.tpl');
            }
        }
    ...
    public function _installDefaultConfig()
    {
        ...
        Configuration::updateValue('PS_PRODUCTS_PER_PAGE_OLD',Configuration::get('PS_PRODUCTS_PER_PAGE'));
        return true;
    }
    public function _unInstallDefaultConfig()
    {
        ...
        if((int)Configuration::get('PS_PRODUCTS_PER_PAGE_OLD'))
            Configuration::updateValue('PS_PRODUCTS_PER_PAGE',Configuration::get('PS_PRODUCTS_PER_PAGE_OLD'));
        else
            Configuration::updateValue('PS_PRODUCTS_PER_PAGE',12);
        if(file_exists(dirname(__FILE__).'/views/css/custom.css'))                
    ...
    {
        return Ets_plp_defines::getInstance()->getConfigInputs();
    }
    ...
    public function saveSubmit($inputs)
    {
        ...
        if (!count($this->_errors)) {
            ...
            if($inputs)
            {
                ...
                Configuration::updateValue('PS_PRODUCTS_PER_PAGE',Configuration::get('ETS_PLP_PRODUCTS_PER_PAGE'));
    ...
    public function _postValidation($inputs)
    {
        ...
        $ETS_PLP_TYPE_PAGINATION = Tools::getValue('ETS_PLP_TYPE_PAGINATION');
        if($ETS_PLP_TYPE_PAGINATION=='load_more')
        {
            $ETS_PLP_BUTTON_LABEL = Tools::getValue('ETS_PLP_BUTTON_LABEL_'.$id_lang_default);
            if(!$ETS_PLP_BUTTON_LABEL)
                $this->_errors[] = $this->l('Button label is required');
            elseif(!Validate::isCleanHtml($ETS_PLP_BUTTON_LABEL))
                $this->_errors[] = $this->l('Button label is not valid');
        }
    }
    public function createCustomCss()
    {
        ...
        $replaces = array(
            'ETS_PLP_BUTTON_LABEL_HOVER' => Configuration::get('ETS_PLP_BUTTON_LABEL_HOVER'),
            'ETS_PLP_BUTTON_BG_HOVER' => Configuration::get('ETS_PLP_BUTTON_BG_HOVER'),
        );
