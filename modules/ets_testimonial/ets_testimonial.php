...
$this->refs = 'https://prestahero.com/';
    ...
    {
        $cacheId = $this->_getCacheId('home_reviews');
        if(!$this->isCached('home_reviews.tpl',$cacheId))
        {
        ...
        return $this->display(__FILE__,'home_reviews.tpl',$cacheId);
    }
    ...
    public function getContent()
    {
        ...
        {
            $fields = array('ETS_TTN_BACKGROUND');
                    ...
                    if($filed_value)
                    {
                        ...
                        foreach($languages as $language)
                        {
                            if($language['id_lang']==$id_lang)
                                ...
                                $values[$language['id_lang']] = Configuration::get($filed,$language['id_lang']);
                        }
                    }
                    Configuration::updateValue($filed,$values);
                    if($filed_value && !in_array($filed_value,$values) && file_exists(_PS_TESTIMONIAL_IMG_DIR_.$filed_value))
                    {
                ...
                elseif($input['type']=='file_lang')
                {
                    ...
                    if(!$this->_errors)
                    {
                        ...
                        if($old_images)
                        {
                            ...
                            {
                                if($image && !in_array($image,$values))
